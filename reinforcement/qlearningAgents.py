...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        listofactions = self.getLegalActions(nextState)
        if listofactions:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        qvalue = 0
        for s in self.featExtractor.getFeatures(state,action).keys():
            qvalue += self.weights[s]*self.featExtractor.getFeatures(state,action)[s]
        return qvalue
