...
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(DIGIT_DATUM_HEIGHT):
        visitmap.append([False]*DIGIT_DATUM_WIDTH)
    for i in range(DIGIT_DATUM_HEIGHT):
        for j in range(DIGIT_DATUM_WIDTH):
            if visitmap[i][j] != True and pixels[i][j] == 0:
                recursioncount(pixels,visitmap,i,j)
                count+=1
    if count>1:
        features['seperate_white_space'] =1
    else:
        features['seperate_white_space'] =0

    # 2.updown symmtric
    symmtric = True
    for i in range(0,14):
        for j in range(DIGIT_DATUM_WIDTH):
            if (pixels[i][j] != pixels[27-i][j]):
                symmtric = False
                break;
    if symmtric:
        features['symmtric_updown'] = 1
    else:
        features['symmtric_updown'] = 0

    return features

def recursioncount(pixels,visitmap,i,j):
    visitmap[i][j]=True
    if i-1>0 and visitmap[i-1][j]!= True and pixels[i-1][j] ==0:
        recursioncount(pixels,visitmap,i-1,j)
    if i+1<28 and visitmap[i+1][j]!= True and pixels[i+1][j] ==0:
        recursioncount(pixels,visitmap,i+1,j)
    if j-1>0 and visitmap[i][j-1]!= True and pixels[i][j-1] ==0:
        recursioncount(pixels,visitmap,i,j-1)
    if j+1<28 and visitmap[i][j+1]!= True and pixels[i][j+1] ==0:
        recursioncount(pixels,visitmap,i,j+1)
...
def enhancedPacmanFeatures(state, action):
    ...
    # move or not
    # print(action)
    # do not have to detect the stop

    # avoid ot not
    disghost = sorted([util.manhattanDistance(successor.getPacmanPosition(),i) for i in successor.getGhostPositions()])
    features["ghost"] = disghost[0] * 1.0

    # food or not
    # print(state.getFood())
    dist =[]
    for food in state.getFood().asList():
        dist.append(util.manhattanDistance(successor.getPacmanPosition(), food))
    closestfood = sorted(dist)
    if not closestfood:
        features['food'] = 0
    else:
        features['food'] = closestfood[0]*1.0
